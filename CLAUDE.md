...
- **`r`/`R` are knowingly inconsistent.** `s`/`S` is next/previous sort column,
  where shift means the counterpart; `r`/`R` is reverse and refresh, two
  unrelated commands sharing a letter. Rebinding them to `r` = refresh and
  `i` = invert was tried and reverted: the flags `-r` (reverse) and `-i`
  (interval) already mean those letters, and disagreeing with your own flags is
  worse than an odd shift pair. Leave them.
...
The screenshots in `docs/` are regenerated with **`nix run .#screenshots`**
from the repo root, which brings its own tmux, termshot and pngquant and
writes the four images in place. It works anywhere, including where the
host's own journal is unreadable: `docs/helpers/fake-journalctl.sh` serves an
invented journal chosen to suit whichever unit is selected, while the unit
table and host stats stay the real machine's. The hostname is replaced on the
way out — keep it that way. `docs/helpers/screenshot.sh` is the single-image
...
- **Screenshots need a readable journal**, which a sandbox or an unprivileged
  account does not have. `docs/helpers/fake-journalctl.sh` first on `PATH` serves an
  invented one, chosen to suit whichever unit is selected; the unit table and
  host stats stay real. That beats screenshotting somebody's production host
  and editing the lines out afterwards.
- **`-H` can be tested without a second machine.** Run an sshd of your own and
  point unitop at it — `docs/helpers/local-sshd.sh` sets one up on 127.0.0.1:2222 with
