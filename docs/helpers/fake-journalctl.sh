...
set -eu
CACHE=${SHOTRIG_CACHE:-${TMPDIR:-/tmp}/unitop-shot-journals}
...
dir=$CACHE/$(printf '%s' "$short" | tr -c 'a-zA-Z0-9._-' '_')
...
6|$short: idle, 3 workers running" ;;
	...
	"$REMOTE" --output="$dir/system.journal" - < "$dir/in.export" >/dev/null 2>&1
fi

eval exec "$REAL" -D "$dir" -q $args
